...
def get_summary(terms: list) -> str: 
    ...
    return terms['content'], response.choices[0].message.content
...
#     ]
    ...
    # Get multiple terms in dataset 
    with open("./test_summary/res_summary/4_1_llm_test_results_CATI-AI_CMC-Legal-LLM-32B-sft-v2.jsonl") as f: 
        laws = [json.loads(line) for line in f.readlines()]
    for terms in tqdm(laws):         
        ...
        with open(f"./16_1/test_summary/res_summary/summary_output_{datetime.now()}.json", "w", encoding="utf-8") as f:
            json.dump({
                "document": doc,
                "summary": summary
        ...
        if args.markdown:
            with open(f"./16_1/test_summary/res_summary/summary_output_{datetime.now()}.md", "w", encoding="utf-8") as f:
                f.write(f"{summary}")